@extends('layouts/homePageLayout')

@section('title', 'الصفحة الرئيسية')

@section('page-style')
<style>
    .product-grid {
        position: relative;
        overflow: hidden;
        border-radius: 5px;
        padding: 10px;
    }

    .item-image {
        /* position: relative;
        overflow: hidden; */
        
    }

    .item-image img {
        /* width: 100%;
        height: auto; */
        transition: opacity 0.3s ease;

        max-width: 100%; /* جعل الصورة تتكيف مع عرض العنصر الأب */
        height: auto; /* الحفاظ على نسبة العرض إلى الارتفاع */
        border-radius: 10px; /* إضافة زوايا مدورة */
    }

    .product-grid:hover img {
        opacity: 0.5; 
    }

    .btns-groupb {
        position: absolute;
        top: 50%;
        left: 50%;
        transform: translate(-50%, -50%);
        opacity: 0; 
        transition: opacity 0.3s ease;
    }

    .product-grid:hover .btns-groub {
        opacity: 1; 
    }

    .btns-groub a {
        font-size: 40px; 
        color: rgba(0, 0, 0, 0.8); 
        text-decoration: none; 
        transition: color 0.3s ease;
    }

    .btns-groub a:hover {
        color: rgba(0, 0, 0, 1);
    }
</style>
    
<style>
    /* إعدادات السلايدر */
    .slider {
        position: relative;
        overflow: hidden;
        width: 100%;
        height: 100vh; /* يغطي الشاشة بالكامل */
    }

    .slider .slide {
        position: absolute;
        top: 0;
        left: 100%;
        width: 100%;
        height: 100%;
        background-size: cover;
        background-position: center;
        transition: all 0.8s ease-in-out;
        display: flex;
        align-items: center;
        justify-content: center;
    }

    .slider .slide.active {
        left: 0;
    }

    .slider .slide.inactive {
        left: -100%;
    }

    .slider .slide::before {
        content: '';
        position: absolute;
        top: 0;
        left: 0;
        width: 100%;
        height: 100%;
        background: rgba(0, 0, 0, 0.5); /* تغميق الصور */
    }

    .slider .slide .content {
        position: relative;
        color: #fff;
        text-align: center;
        padding: 20px;
        z-index: 1;
    }

    .slider .slide h1 {
        color: #ffffff;
        font: bold;
        font-size: 4rem;
        margin-bottom: 20px;
    }

    .slider .slide p {
        font-size: 1.5rem;
        line-height: 1.8;
        max-width: 800px;
        margin: 0 auto;
    }

    /* النقاط */
    .dots {
        position: absolute;
        bottom: 20px;
        left: 50%;
        transform: translateX(-50%);
        display: flex;
        gap: 10px;
        z-index: 2;
    }

    .dots button {
        background: #fff;
        border: none;
        border-radius: 50%;
        width: 15px;
        height: 15px;
        cursor: pointer;
        opacity: 0.5;
        transition: opacity 0.3s ease;
    }

    .dots button.active {
        opacity: 1;
    }

    /* استجابة الشاشات */
    @media (max-width: 768px) {
        .slider {
            height: 70vh;
        }

        .slider .slide h1 {
            font-size: 2rem;
        }

        .slider .slide p {
            font-size: 1rem;
        }
    }
</style>

@endsection
@section('layoutContent')
<!-- backtotop - start -->
<div id="thetop"></div>
<div id="backtotop">
    <a href="#" id="scroll">
        <i class="las la-arrow-up"></i>
    </a>
</div>



<main>

    {{-- <section id="slider-section" class="slider-section home-two-slider clearfix " style="direction: ltr;">
        <div id="main-carousel" class="main-carousel arrow-right-left owl-carousel owl-theme" style="direction: ltr;">
    
            <div class="item d-flex align-items-center bg-gray">
                <div class="container">
                    <div class="row align-items-center justify-content-lg-end justify-content-md-between"> <!-- تغيير من start إلى end -->
                        <div class="col-lg-5 col-md-6 col-sm-12 col-xs-12 order-first"> <!-- تغيير من order-last إلى order-first -->
                            <div class="slider-image meter-image">
                                <img src="{{asset('assets/home/images/slider/im1.jpg')}}" alt="image_not_found">
                            </div>
                        </div>
    
                        <div class="col-lg-7 col-md-6 col-sm-12 col-xs-12">
                            <div class="slider-content text-right"> <!-- إضافة text-right -->
                                <span class="off-price animate-item mb-15">25% Off</span>
                                <h3 class="item-title animate-item">
                                    ميزان حرارة رقمي للجبهة بدون تلامس.
                                </h3>
                                <p class="animate-item">
                                    ميديسيا دائمًا ما تركز على أفضل العلاجات بأسعار معقولة من خلال أفضل الخدمات الطبية.
                                </p>
                                <div class="btns-group ul-li animate-item clearfix">
                                    <ul class="clearfix">
                                        <li><a href="#!" class="btn bg-light-green">تسوق الآن</a></li>
                                        <li><a href="#!" class="btn bg-default-black">تعرف أكثر</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
    
            <div class="item d-flex align-items-center bg-gray">
                <div class="container">
                    <div class="row align-items-center justify-content-lg-end justify-content-md-between"> <!-- تغيير من start إلى end -->
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 order-first"> <!-- تغيير من order-last إلى order-first -->
                            <div class="slider-image mask-image">
                                <img src="{{asset('assets/home/images/slider/im3.jpg')}}" alt="image_not_found">
                            </div>
                        </div>
    
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            <div class="slider-content text-right"> <!-- إضافة text-right -->
                                <span class="off-price animate-item mb-15">30% off</span>
                                <h3 class="item-title animate-item">
                                    قناع غبار NIOSH N95 قابل للطي
                                </h3>
                                <p class="animate-item">
                                    ميديسيا دائمًا ما تركز على أفضل العلاجات بأسعار معقولة من خلال أفضل الخدمات الطبية.
                                </p>
                                <div class="btns-group ul-li animate-item clearfix">
                                    <ul class="clearfix">
                                        <li><a href="#!" class="btn bg-light-green">تسوق الآن</a></li>
                                        <li><a href="#!" class="btn bg-default-black">تعرف أكثر</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
    
            <div class="item d-flex align-items-center bg-gray">
                <div class="container">
                    <div class="row align-items-center justify-content-lg-end justify-content-md-between"> <!-- تغيير من start إلى end -->
                        <div class="col-lg-5 col-md-6 col-sm-12 col-xs-12 order-first"> <!-- تغيير من order-last إلى order-first -->
                            <div class="slider-image meter-image">
                                <img src="{{asset('assets/home/images/slider/meter_1.png')}}" alt="image_not_found">
                            </div>
                        </div>
    
                        <div class="col-lg-7 col-md-6 col-sm-12 col-xs-12">
                            <div class="slider-content text-right"> <!-- إضافة text-right -->
                                <span class="off-price animate-item mb-15">25% Off</span>
                                <h3 class="item-title animate-item">
                                    ميزان حرارة رقمي للجبهة بدون تلامس.
                                </h3>
                                <p class="animate-item">
                                    ميديسيا دائمًا ما تركز على أفضل العلاجات بأسعار معقولة من خلال أفضل الخدمات الطبية.
                                </p>
                                <div class="btns-group ul-li animate-item clearfix">
                                    <ul class="clearfix">
                                        <li><a href="#!" class="btn bg-light-green">عرض</a></li>
                                        <li><a href="#!" class="btn bg-default-black">تعرف أكثر</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
    
            <div class="item d-flex align-items-center bg-gray">
                <div class="container">
                    <div class="row align-items-center justify-content-lg-end justify-content-md-between"> <!-- تغيير من start إلى end -->
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 order-first"> <!-- تغيير من order-last إلى order-first -->
                            <div class="slider-image mask-image">
                                <img src="{{asset('assets/home/images/slider/im2.jpg')}}" alt="image_not_found">
                            </div>
                        </div>
    
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            <div class="slider-content text-right"> <!-- إضافة text-right -->
                                <span class="off-price animate-item mb-15">30% off</span>
                                <h3 class="item-title animate-item">
                                    قناع غبار NIOSH N95 قابل للطي
                                </h3>
                                <p class="animate-item">
                                    ميديسيا دائمًا ما تركز على أفضل العلاجات بأسعار معقولة من خلال أفضل الخدمات الطبية.
                                </p>
                                <div class="btns-group ul-li animate-item clearfix">
                                    <ul class="clearfix">
                                        <li><a href="#!" class="btn bg-light-green">تسوق الآن</a></li>
                                        <li><a href="#!" class="btn bg-default-black">تعرف أكثر</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
    
        </div>
    </section> --}}
    {{-- <section id="slider-section" class="slider-section home-two-slider clearfix" style="direction: ltr;">
        <div id="main-carousel" class="main-carousel arrow-right-left owl-carousel owl-theme" style="direction: ltr;" data-owl-autoplay="true" data-owl-autoplay-timeout="5000" data-owl-nav="true" data-owl-dots="true">
        
            <!-- الشريحة الأولى -->
            <div class="item d-flex align-items-center" style="background-image: url('{{asset('assets/home/images/slider/im1.jpg')}}'); background-size: cover; background-position: center; height: 400px;">
                <div class="container">
                    <div class="row align-items-center justify-content-lg-end justify-content-md-between">
                        <div class="col-lg-5 col-md-6 col-sm-12 col-xs-12 order-first">
                            <!-- تم حذف الصورة القديمة هنا -->
                        </div>
                        <div class="col-lg-7 col-md-6 col-sm-12 col-xs-12">
                            <div class="slider-content text-right">
                                <span class="off-price animate-item mb-15">25% Off</span>
                                <h3 class="item-title animate-item">
                                    ميزان حرارة رقمي للجبهة بدون تلامس.
                                </h3>
                                <p class="animate-item">
                                    ميديسيا دائمًا ما تركز على أفضل العلاجات بأسعار معقولة من خلال أفضل الخدمات الطبية.
                                </p>
                                <div class="btns-group ul-li animate-item clearfix">
                                    <ul class="clearfix">
                                        <li><a href="#!" class="btn bg-light-green">تسوق الآن</a></li>
                                        <li><a href="#!" class="btn bg-default-black">تعرف أكثر</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
    
            <!-- الشريحة الثانية -->
            <div class="item d-flex align-items-center" style="background-image: url('{{asset('assets/home/images/slider/im3.jpg')}}'); background-size: cover; background-position: center; height: 400px;">
                <div class="container">
                    <div class="row align-items-center justify-content-lg-end justify-content-md-between">
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 order-first">
                            <!-- تم حذف الصورة القديمة هنا -->
                        </div>
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            <div class="slider-content text-right">
                                <span class="off-price animate-item mb-15">30% off</span>
                                <h3 class="item-title animate-item">
                                    قناع غبار NIOSH N95 قابل للطي
                                </h3>
                                <p class="animate-item">
                                    ميديسيا دائمًا ما تركز على أفضل العلاجات بأسعار معقولة من خلال أفضل الخدمات الطبية.
                                </p>
                                <div class="btns-group ul-li animate-item clearfix">
                                    <ul class="clearfix">
                                        <li><a href="#!" class="btn bg-light-green">تسوق الآن</a></li>
                                        <li><a href="#!" class="btn bg-default-black">تعرف أكثر</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
    
            <!-- الشريحة الثالثة -->
            <div class="item d-flex align-items-center" style="background-image: url('{{asset('assets/home/images/slider/meter_1.png')}}'); background-size: cover; background-position: center; height: 400px;">
                <div class="container">
                    <div class="row align-items-center justify-content-lg-end justify-content-md-between">
                        <div class="col-lg-5 col-md-6 col-sm-12 col-xs-12 order-first">
                            <!-- تم حذف الصورة القديمة هنا -->
                        </div>
                        <div class="col-lg-7 col-md-6 col-sm-12 col-xs-12">
                            <div class="slider-content text-right">
                                <span class="off-price animate-item mb-15">25% Off</span>
                                <h3 class="item-title animate-item">
                                    ميزان حرارة رقمي للجبهة بدون تلامس.
                                </h3>
                                <p class="animate-item">
                                    ميديسيا دائمًا ما تركز على أفضل العلاجات بأسعار معقولة من خلال أفضل الخدمات الطبية.
                                </p>
                                <div class="btns-group ul-li animate-item clearfix">
                                    <ul class="clearfix">
                                        <li><a href="#!" class="btn bg-light-green">عرض</a></li>
                                        <li><a href="#!" class="btn bg-default-black">تعرف أكثر</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
    
            <!-- الشريحة الرابعة -->
            <div class="item d-flex align-items-center" style="background-image: url('{{asset('assets/home/images/slider/im2.jpg')}}'); background-size: cover; background-position: center; height: 400px;">
                <div class="container">
                    <div class="row align-items-center justify-content-lg-end justify-content-md-between">
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 order-first">
                            <!-- تم حذف الصورة القديمة هنا -->
                        </div>
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            <div class="slider-content text-right">
                                <span class="off-price animate-item mb-15">30% off</span>
                                <h3 class="item-title animate-item">
                                    قناع غبار NIOSH N95 قابل للطي
                                </h3>
                                <p class="animate-item">
                                    ميديسيا دائمًا ما تركز على أفضل العلاجات بأسعار معقولة من خلال أفضل الخدمات الطبية.
                                </p>
                                <div class="btns-group ul-li animate-item clearfix">
                                    <ul class="clearfix">
                                        <li><a href="#!" class="btn bg-light-green">تسوق الآن</a></li>
                                        <li><a href="#!" class="btn bg-default-black">تعرف أكثر</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
    
        </div>
    </section> --}}

    {{-- <section id="slider-section" class="slider-section home-two-slider clearfix" style="direction: ltr;">
        <div id="main-carousel" class="main-carousel arrow-right-left owl-carousel owl-theme" style="direction: ltr;" data-owl-autoplay="true" data-owl-autoplay-timeout="5000" data-owl-nav="true" data-owl-dots="true">
        
            <!-- الشريحة الأولى -->
            <div class="item d-flex align-items-center" style="background-image: url('{{asset('assets/home/images/slider/im1.jpg')}}'); background-size: cover; background-position: center; height: 500px;">
                <div class="container">
                    <div class="row align-items-center justify-content-center">
                        <div class="col-lg-6 col-md-8 col-sm-12 text-center">
                            <div class="slider-content">
                                <h3 class="item-title animate-item" style="color: #fff; font-size: 40px; font-weight: bold;">
                                    الابتكار في صناعة الأدوية لجعل العالم أكثر صحة
                                </h3>
                                <p class="animate-item" style="color: #fff; font-size: 18px; line-height: 1.7;">
                                    شركة العالمية المثالية للأدوية ملتزمة بتقديم حلول طبية مبتكرة، تحرص على توفير أدوية عالية الجودة لدعم رفاهية الإنسان حول العالم.
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
    
            <!-- الشريحة الثانية -->
            <div class="item d-flex align-items-center" style="background-image: url('{{asset('assets/home/images/slider/im3.jpg')}}'); background-size: cover; background-position: center; height: 500px;">
                <div class="container">
                    <div class="row align-items-center justify-content-center">
                        <div class="col-lg-6 col-md-8 col-sm-12 text-center">
                            <div class="slider-content">
                                <h3 class="item-title animate-item" style="color: #fff; font-size: 40px; font-weight: bold;">
                                    أدوية مبتكرة لدعم المستقبل الصحي
                                </h3>
                                <p class="animate-item" style="color: #fff; font-size: 18px; line-height: 1.7;">
                                    نركز على تطوير أدوية متطورة وفعالة، مع التزامنا الدائم بتحقيق التميز في كل ما نقدمه لعملائنا.
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- الشريحة الرابعة -->
            <div class="item d-flex align-items-center" style="background-image: url('{{asset('assets/home/images/slider/im2.jpg')}}'); background-size: cover; background-position: center; height: 500px;">
                <div class="container">
                    <div class="row align-items-center justify-content-center">
                        <div class="col-lg-6 col-md-8 col-sm-12 text-center">
                            <div class="slider-content">
                                <h3 class="item-title animate-item" style="color: #fff; font-size: 40px; font-weight: bold;">
                                    ابتكار وقيادة في صناعة الأدوية
                                </h3>
                                <p class="animate-item" style="color: #fff; font-size: 18px; line-height: 1.7;">
                                    من خلال البحث المتقدم والجودة العالية، تواصل شركة العالمية المثالية للأدوية ريادتها في توفير حلول طبية تعزز الصحة العامة.
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
    
        </div>
    </section> --}}
    

    <div class="slider" id="slider">
        <!-- الشريحة الأولى -->
        <div class="slide active" style="background-image: url('{{asset('assets/home/images/slider/im1.jpg')}}');">
            <div class="content">
                <h1>شركة العالمية المثالية للأدوية</h1>
                <p>
                    نحن في العالمية المثالية للأدوية نسعى لتقديم أفضل الحلول العلاجية لعملائنا حول العالم.
                    الجودة والابتكار هما رؤيتنا لتطوير مستقبل الصحة.
                </p>
            </div>
        </div>

        <!-- الشريحة الثانية -->
        <div class="slide" style="background-image: url('{{asset('assets/home/images/slider/im3.jpg')}}');">
            <div class="content">
                <h1>ابتكار في تطوير الأدوية</h1>
                <p>
                    نلتزم بتطوير أدوية حديثة ومبتكرة تلبي احتياجات المرضى والمجتمعات من خلال التكنولوجيا المتقدمة.
                </p>
            </div>
        </div>

        <!-- الشريحة الثالثة -->
        <div class="slide" style="background-image: url('{{asset('assets/home/images/slider/im2.jpg')}}');">
            <div class="content">
                <h1>شريكك الصحي الأول</h1>
                <p>
                    شراكتنا مع المراكز الصحية تضمن تقديم منتجات عالية الجودة تخدم المرضى حول العالم.
                </p>
            </div>
        </div>

        <!-- النقاط -->
        <div class="dots" id="dots"></div>
    </div>



   
    
   
    
    
    
    <section class="custom-section">
        <div class="container content">
            <div class="row align-items-center">
                <div class="col-lg-6 text-left">
                    <img src="https://www.acdima.com/upload/Rectangle_4511.jpg" alt="مبنى أكديما" class="img-fluid">
                </div>
                <div class="col-lg-6 mb-4 mb-lg-0  p-5">
                    <h2>عن الشركة العالمية المثالية للأدوية</h2>
                    <p>
                        الشركة العالمية المثالية للأدوية هي شركة رائدة في مجال تصنيع الأدوية والمستلزمات الطبية، مقرها الرئيسي في صنعاء، اليمن. تأسست الشركة بهدف توفير أدوية عالية الجودة بأسعار معقولة، مع التركيز على تحسين الرعاية الصحية للمجتمع. تتميز الشركة باتباعها أعلى معايير الجودة في التصنيع، وتوظيف أحدث التقنيات لضمان فعالية وأمان منتجاتها.
                    </p>
                    <a href="#" class="btn btn-primary mt-3">المزيد</a>
                </div>
                
            </div>
        </div>
    </section>


    
    <section id="promotion-section" class="promotion-section sec-ptb-100 pb-0 clearfix">
        <div class="container">
            <div class="masonry-grid">
                <div class="grid-sizer"></div>
    
                <div class="grid-item">
                    <div class="promotion-fullimage clearfix">
                        <a href="#!" class="item-image">
                            <img src="{{asset('assets/home/images/promotion/img_5.jpg')}}" alt="image_not_found">
                        </a>
                        <div class="promotion-content position-top size-decrease">
                            <small class="d-block text-white mb-1">مستلزمات طبية</small>
                            <h3 class="item-title">
                                <span class="d-block">مستلزمات طبية</span> <span class="d-block">لفيروس كورونا</span>
                            </h3>
                            <a href="#!" class="btn-underline">تسوق الآن</a>
                        </div>
                    </div>
                </div>
    
                <div class="grid-item w-66">
                    <div class="promotion-fullimage clearfix">
                        <a href="#!" class="item-image">
                            <img src="{{asset('assets/home/images/promotion/img_6.jpg')}}" alt="image_not_found">
                        </a>
                        <div class="promotion-content size-increase position-vertical-center">
                            <h3 class="item-title">
                                {{-- <strong class="text-royal-blue d-block mb-1">خصم 50%</strong>  --}}
                                <span class="d-block">جميع الادوية متوفرة</span> 
                                {{-- <span class="d-block">للشراء الأول</span> --}}
                            </h3>
                            <a href="#!" class="btn bg-royal-blue">تسوق الآن</a>
                        </div>
                    </div>
                </div>
    
            </div>
        </div>
    </section>

    <section id="category-section" class="category-section sec-ptb-100 pb-0 clearfix">
        <div class="container wrap-1500">
            <div class="row mt--30 justify-content-lg-between justify-content-md-center justify-content-sm-center">

                <div class="col-lg-2 col-md-3 col-sm-4 col-6">
                    <a href="index.html" class="category-primary text-center clearfix">
                        <div class="item-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="60" height="75" viewBox="0 0 60 75">
                                    <image width="60" height="75" xlink:href="data:img/png;base64,"></image>
                                </svg>
                        </div>
                        <span class="item-title">الأجهزة الطبية</span>
                    </a>
                </div>

                <div class="col-lg-2 col-md-3 col-sm-4 col-6">
                    <a href="index.html" class="category-primary text-center clearfix">
                        <div class="item-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="72" height="72" viewBox="0 0 72 72">
                                    <image width="72" height="72" xlink:href="data:img/png;base64,"></image>
                                </svg>
                        </div>
                        <span class="item-title">الوقاية من كوفيد-19</span>
                    </a>
                </div>

                <div class="col-lg-2 col-md-3 col-sm-4 col-6">
                    <a href="index.html" class="category-primary text-center clearfix">
                        <div class="item-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="77" height="61" viewBox="0 0 77 61">
                                    <image width="77" height="61" xlink:href="data:img/png;base64,"></image>
                                </svg>
                        </div>
                        <span class="item-title">أدوات الجراحة</span>
                    </a>
                </div>

                <div class="col-lg-2 col-md-3 col-sm-4 col-6">
                    <a href="index.html" class="category-primary text-center clearfix">
                        <div class="item-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="68" height="67" viewBox="0 0 68 67">
                                    <image width="68" height="67" xlink:href="data:img/png;base64,"></image>
                                </svg>
                        </div>
                        <span class="item-title">الطب العام</span>
                    </a>
                </div>

                <div class="col-lg-2 col-md-3 col-sm-4 col-6">
                    <a href="index.html" class="category-primary text-center clearfix">
                        <div class="item-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="58" height="70" viewBox="0 0 58 70">
                                    <image width="58" height="70" xlink:href="data:img/png;base64,"></image>
                                </svg>
                        </div>
                        <span class="item-title">الصحة واللياقة</span>
                    </a>
                </div>

                <div class="col-lg-2 col-md-3 col-sm-4 col-6">
                    <a href="index.html" class="category-primary text-center clearfix">
                        <div class="item-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="66" height="66" viewBox="0 0 66 66">
                                    <image width="66" height="66" xlink:href="data:img/png;base64,"></image>
                                </svg>
                        </div>
                        <span class="item-title">الأدوات الطبية</span>
                    </a>
                </div>

            </div>
        </div>
    </section>

    
    <section class="shop-section sec-ptb-100 clearfix">
        <div class="container">
            <div class="section-title text-center mb-70">
                <h2 class="title-text mb-3">المنتجات حسب الفئة</h2>
                <p class="mb-0">تسوق فوق 59 دولار أو أول عملية شراء تحصل على شحن مجاني 100%</p>
            </div>
    
            <div class="tabs-nav ul-li-center clearfix">
                <ul class="nav" role="tablist">
                    {{-- <li>
                        <a href="#!" class="category-tab active" data-category-id="">كل المنتجات</a>
                    </li> --}}
                    @foreach ($categories as $category)
                        <li>
                            <a href="#!" class="category-tab" data-category-id="{{ $category->id }}">{{ $category->name }}</a>
                        </li>
                    @endforeach
                </ul>
            </div>
    
            <div class="tab-content">
                <div id="products" class="tab-pane active">
                    <div class="row justify-content-center mb-70" id="product-container">
                        @foreach ($products as $product)
                            <div class="col-lg-3 col-md-4 col-sm-6 col-xs-12">
                                <div class="product-grid text-center clearfix">
                                    <div class="item-image">
                                        <a href="" class="image-wrap">
                                            <img src="{{ asset($product->image_url) }}" alt="{{ $product->name }}">
                                        </a>
                                    </div>
                                    <div class="item-content">
                                        <h3 class="item-title">
                                            <a href="">{{ $product->name }}</a>
                                        </h3>
                                        <span class="item-price">${{ $product->price }}</span>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    <div class="btn-wrap text-center clearfix">
                        <a href="#!" class="btn bg-royal-blue">عرض جميع المنتجات</a>
                    </div>
                </div>
            </div>
        </div>
    </section>
    

     
    <!-- offer-section - start -->
    

    
    <!-- blog-section - start -->
    <section id="blog-section" class="blog-section sec-ptb-100 clearfix">
        <div class="container">
    
            <div class="section-title text-center mb-40">
                <h2 class="title-text mb-3">من مدونتنا</h2>
                <p class="mb-0">تحقق من أحدث المقالات من صفحات المدونة حول الصحة والفحص الطبي والأدوية</p>
            </div>
    
            <div class="row justify-content-lg-between justify-content-md-center justify-content-sm-center">
                @foreach ($posts as $post)
                <div class="col-lg-4 col-md-12 col-sm-12 col-xs-12">
                    <div class="blog-boxed" data-background="{{asset('assets/home/images/blog/img_1.jpg')}}">
                       
                        <div class="post-admin clearfix">
                        </div>
                        <h3 class="item-title mb-3">
                            <a href="#!"> {{$post->title}}</a>
                        </h3>
                        <p>
                            {{$post->description}}
                        </p>
                        <a href="#!" class="btn-underline">اقرأ المزيد</a>
                       
                    </div>
                   
                </div>
                @endforeach
    
                <!-- Repeat for other blog posts -->
            </div>
    
        </div>
    </section>
    
    <!-- brand-section - start -->
    <div id="brand-section" class="brand-section clearfix">
        <div class="container">
            <div class="brand-carousel mt--40 owl-carousel owl-theme">
    
                <div class="item">
                    <a class="brand-logo" href="#!">
                        <img src="{{asset('assets/home/images/brand/img_1.png')}}" alt="logo_not_found">
                    </a>
                </div>
    
                <!-- Repeat for other brand logos -->
            </div>
        </div>
    </div>

</main>



    <script src="{{ asset('assets/home/js/jquery-3.4.1.min.js') }}"></script> 
    <script src="{{ asset('assets/home/js/jquery-ui.js') }}"></script>
    <script src="{{ asset('assets/home/js/popper.min.js') }}"></script>
    <script src="{{ asset('assets/home/js/bootstrap.min.js') }}"></script>
    <script src="{{ asset('assets/home/js/magnific-popup.min.js') }}"></script>
    <script src="{{ asset('assets/home/js/owl.carousel.min.js') }}"></script>
    <script src="{{ asset('assets/home/js/owl.carousel2.thumbs.min.js') }}"></script>
    <script src="{{ asset('assets/home/js/isotope.pkgd.min.js') }}"></script>
    <script src="{{ asset('assets/home/js/masonry.pkgd.min.js') }}"></script>
    <script src="{{ asset('assets/home/js/imagesloaded.pkgd.min.js') }}"></script>
    <script src="{{ asset('assets/home/js/countdown.js') }}"></script>

    <!-- Google Maps JS -->
    <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDk2HrmqE4sWSei0XdKGbOMOHN3Mm2Bf-M&ver=2.1.6"></script>
    <script src="{{ asset('assets/home/js/gmaps.min.js') }}"></script>

    <!-- Mobile Menu JS -->
    <script src="{{ asset('assets/home/js/mCustomScrollbar.js') }}"></script>

    <!-- Custom JS -->
    <script src="{{ asset('assets/home/js/custom.js') }}"></script>

    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const categoryTabs = document.querySelectorAll('.category-tab');
            const productContainer = document.getElementById('product-container');
    
            // دالة لتحميل المنتجات بناءً على التصنيف
            function loadProducts(categoryId = '') {
                fetch(`/products-by-category?category_id=${categoryId}`)
                    .then(response => response.json())
                    .then(data => {
                        // تفريغ المنتجات الحالية
                        productContainer.innerHTML = '';
    
                        // إضافة المنتجات الجديدة
                        if (data.products.length > 0) {
                            data.products.forEach(product => {
                                productContainer.innerHTML += `
                                    <div class="col-lg-3 col-md-4 col-sm-6 col-xs-12">
                                        <div class="product-grid text-center clearfix">
                                            <div class="item-image">
                                                <a href="/product-details/${product.id}" class="image-wrap">
                                                    <img src="${product.image_url}" alt="${product.name}">
                                                </a>
                                            </div>
                                            <div class="item-content">
                                                <h3 class="item-title">
                                                    <a href="/product-details/${product.id}">${product.name}</a>
                                                </h3>
                                            </div>
                                        </div>
                                    </div>
                                `;
                            });
                        } else {
                            productContainer.innerHTML = '<p class="text-center p-5">لا توجد منتجات متاحة.</p>';
                        }
                    })
                    .catch(error => console.error('Error fetching products:', error));
            }
    
            // تحميل المنتجات الافتراضية عند فتح الصفحة
            loadProducts();
    
            // تحديث المنتجات عند النقر على التصنيفات
            categoryTabs.forEach(tab => {
                tab.addEventListener('click', function (e) {
                    e.preventDefault();
    
                    // إزالة التنشيط عن جميع التبويبات وتفعيل الحالي
                    categoryTabs.forEach(tab => tab.classList.remove('active'));
                    this.classList.add('active');
    
                    // الحصول على معرف التصنيف
                    const categoryId = this.getAttribute('data-category-id');
    
                    // تحميل المنتجات بناءً على التصنيف
                    loadProducts(categoryId);
                });
            });
        });
    </script>
   

{{-- <script>
    function applyResponsiveStyles() {
        const screenWidth = window.innerWidth;
        const slider = document.getElementById('slider');
        const sliderTitle = document.getElementById('slider-title');

        if (screenWidth < 768) { // شاشات صغيرة
            slider.style.height = '300px';
            sliderTitle.style.fontSize = '20px';
        } else { // شاشات كبيرة
            slider.style.height = '500px';
            sliderTitle.style.fontSize = '40px';
        }
    }

    // تطبيق الأنماط عند تحميل الصفحة وتغيير حجم الشاشة
    applyResponsiveStyles();
    window.addEventListener('resize', applyResponsiveStyles);
</script> --}}

<script>
    // عناصر السلايدر
    const slides = document.querySelectorAll('.slide');
    const dotsContainer = document.getElementById('dots');
    let currentSlide = 0;

    // إنشاء النقاط بناءً على عدد الشرائح
    slides.forEach((_, index) => {
        const dot = document.createElement('button');
        dot.addEventListener('click', () => goToSlide(index));
        dotsContainer.appendChild(dot);
    });

    const dots = dotsContainer.querySelectorAll('button');
    dots[0].classList.add('active');

    // تغيير الشريحة
    function goToSlide(index) {
        slides[currentSlide].classList.remove('active');
        dots[currentSlide].classList.remove('active');
        currentSlide = index;
        slides[currentSlide].classList.add('active');
        dots[currentSlide].classList.add('active');
    }

    // الانتقال التلقائي بين الشرائح
    function autoSlide() {
        goToSlide((currentSlide + 1) % slides.length);
    }

    setInterval(autoSlide, 5000); // تغيير كل 5 ثوانٍ
</script>

    
@endsection